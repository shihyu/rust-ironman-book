...
- 原文連結：https://ithelp.ithome.com.tw/articles/10277734
- 章節：DAY 27
- 發佈時間：2021-10-05 08:56:42
- 作者：Ken
- 清單瀏覽數：1814
- 清單摘要：有關於並行和並發的定義每個人可能有不一樣的解釋 並行指的是在同一時刻，多條指令在 CPU 上同時執行 並發指的是在同一時間區間內，多條指令在 CPU 上同時執...
...
![](images/day-27-rust-並行-並發-一/img-001.jpg)
...
─────────────── 執行緒關係 ──────────────────────
  ...
  main thread
  ┌──────────────────────────────────┐
  │ spawn(move || worker())          │    ← 建立子執行緒
  │ handle.join()                    │    ← 等待子執行緒結束
  └────────────────┬─────────────────┘
                   │
                   v
  ┌──────────────────────────────────┐
