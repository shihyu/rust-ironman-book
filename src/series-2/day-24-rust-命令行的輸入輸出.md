...
- 原文連結：https://ithelp.ithome.com.tw/articles/10276239
- 章節：DAY 24
- 發佈時間：2021-10-02 09:46:01
- 作者：Ken
- 清單瀏覽數：1261
- 清單摘要：命令行參數 一般來說編譯好的執行檔都是透過命令行來執行有些時候需要讀取一些命令行參數或是環境參數 程式輸入的最基本形式是透過命令行 例如下面 for arg i...

## 內文

### 命令行參數

一般來說編譯好的執行檔都是透過命令行來執行有些時候需要讀取一些命令行參數或是環境參數
...
剛剛是把每個環境變數都印出來假如要讀取特定環境變數可以透過var函式指定key
...
═══════════════════════════════════════════════
  ...
  └────────────┬─────────────┘
               ...
               v
  ┌──────────────────────────┐
  │  stdin.read_line()       │         ← 互動式輸入
  │  互動式輸入               │
  └────────────┬─────────────┘
               │
               v
  ┌──────────────────────────┐
  │  stdout / stderr 輸出     │         ← 結果輸出
  └──────────────────────────┘
```
