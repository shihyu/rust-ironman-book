...
- 原文連結：https://ithelp.ithome.com.tw/articles/10269238
- 章節：DAY 17
- 發佈時間：2021-09-25 11:28:25
- 作者：Ken
- 清單瀏覽數：1326
- 清單摘要：函式傳遞參數 每次調用函式時都打印相同的hello world的函式不是很有用處 這時候可以傳遞參數給函式會顯得更有意義 fn hello_word(name:...

## 內文

### 函式傳遞參數

每次調用函式時都打印相同的hello world的函式不是很有用處
...
f(5u16); // 非法因為是u16型別
f(5i16); // 合法
f(5); // 合法因為傳遞是無約束的整數類型參數會變函式限制為i16類型
```
...
在一開始就宣告的f變數在傳遞給test函式的，並在函式中保留使用f變數並更改變數的值並打印，函式結束返給調用者，然後再印出f變數，這個變數與調用時一樣
