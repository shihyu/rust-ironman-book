...
- 原文連結：https://ithelp.ithome.com.tw/articles/10275658
- 章節：DAY 23
- 發佈時間：2021-10-01 08:44:33
- 作者：Ken
- 清單瀏覽數：1039
- 清單摘要：一般來說Rust如果要排序數組會這樣寫 let mut arr = [10, 5, 9, 7, 6] arr.sort(); println!("...
...
剛剛升序如果要降序就必須使用sort_by
需要自己寫函式來傳給sort_by排序決定用
...
─────────────── Closure 環境物件 ────────────────
  ...
  ┌────────────────────────────────────────┐
  │ captured x: by & / by mut & / move     │ ← 依使用情況推導
  │ captured s: 依使用情況推導              │ ← 編譯器自動決定
  └───────────────────┬────────────────────┘
                      │
                      v
               call closure(...)
