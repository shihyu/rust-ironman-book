...
- 原文連結：https://ithelp.ithome.com.tw/articles/10272643
- 章節：DAY 20
- 發佈時間：2021-09-30 08:46:53
- 作者：Ken
- 清單瀏覽數：1077
- 清單摘要：所有權與函式 將一個變數當作函式的參數傳給其他函式，怎樣安全的處理所有權 傳遞數值給函式這樣的語義和賦值給變數是類似的。傳遞變數給函式會是移動或拷貝就像賦值一樣...
...
caller
  │
  ├──> take(name)      : name moved, caller 不可再用    ← 所有權移入
  │
  ├──> borrow(&name2)  : 只讀借用, caller 仍擁有        ← 僅借用
  │
  └<── give()          : 新所有權回到 caller             ← 所有權移出
