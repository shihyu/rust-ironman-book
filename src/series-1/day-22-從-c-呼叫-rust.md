...
- 原文連結：https://ithelp.ithome.com.tw/articles/10204232
- 章節：DAY 22
- 發佈時間：2018-10-22 22:40:03
- 作者：DanSnow
- 清單瀏覽數：3176
- 清單摘要：這次的程式碼在 https://github.com/DanSnow/rust-intro/tree/master/clib 建立可以給 C 使用的函式庫 Ru...
...
Rust 跟 C 真的是個很合的語言，要從 C 呼叫 Rust
的程式也很簡單，這次我們來建立一個之前都沒有使用過的函式庫專案：
...
這代表我們要 cargo 建置出可以用來做靜態連結的函式庫，接著來準備個給 C
使用的函式吧，打開 `lib.rs` 輸入：
...
> Rust 的編譯器會修改函式名稱，加上模組等資訊來避免出現重覆的名稱，另外
> Rust 的調用約定 (calling convention) 也與 C 不同，因此必須加上
> `extern "C"` 讓 Rust 使用 C 的調用約定，這樣我們才能直接在 C
> 使用這些函式。
...
這次我們一樣為了簡化編譯的過程把這部份寫成了一個 `Makefile`
，可以自己打開來看看，執行的話應該會看到：
...
> 務必讓 Rust 清理記憶體， Rust 預設並不是使用 malloc 與 free ，若用
> free 來清理是會出問題的，再者，只有 Rust
> 知道那個型態有沒有其它需要釋放的資源。

## 傳 Vec

Rust 的 `Vec`
真的很方便，可以自動的成長，做為陣列使用就不用擔心空間不夠的問題
(除非你的環境的記憶體很珍貴) ，如果要傳遞給 C 使用的話要怎麼辦呢：
...
前面我們都自己宣告 C
的函式，這次讓程式來自動幫我們產生標頭檔吧，首先加上 `cbindgen` ：
